...
suite('alias resolver', () => {
  test('loads tsconfig paths and resolves wildcard @/*', () => {
    const fixtureRoot = path.resolve(process.cwd(), 'src', 'test', 'fixtures', 'projectA');
    const cfg = loadAliasesForRoot(fixtureRoot);
    assert.ok(cfg);
    const cand = resolveAliasToFiles('@/assets/images/link_icon_x.svg', vscode.Uri.file(path.join(fixtureRoot, 'src', 'app.ts')));
  ...
  test('resolves @images/* to assets/images', () => {
    const fixtureRoot = path.resolve(process.cwd(), 'src', 'test', 'fixtures', 'projectA');
    const cand = resolveAliasToFiles('@images/logo.png', vscode.Uri.file(path.join(fixtureRoot, 'src', 'app.ts')));
